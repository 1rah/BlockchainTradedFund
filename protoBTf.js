/** The protoBTF Token - BTF preICO code.
 *  Raised Ether will be stored safely at the wallet and automatically returned to pre-ICO purchasers in case the 
 *  funding goal is not reached, allowing coin purchasers to withdraw their funds.**/
 ...
 * Provides onlyOwner modifier to prevent function from running if it is called by anyone other than the owner.
 */
contract Ownable {
...
contract BTFpreSale is Haltable, SafeMath {
     ...
     *  Constants
    /*/
    string public constant name = "protoBTf BTf Presale Token";

    string public constant symbol = "pBTf";

    uint   public constant decimals = 18;

    // 5% of tokens... uint256 public constant SUPPLY_LIMIT = 5000000 ether;
    uint256 public constant SUPPLY_LIMIT = 5000000;

    // What is given to contributors, <= SUPPLY_LIMIT
    uint256 public totalSupply;

    // If soft cap is not reached, refund process is started
    uint256 public softCap = 4 ether;

    // Basic price
    uint256 public constant basicThreshold = 500 finney;

    uint public constant basicTokensPerEth = 2000;

    // Advanced price
    uint256 public constant advancedThreshold = 5 ether;

    uint public constant advancedTokensPerEth = 3000;

    // Expert price
    uint256 public constant expertThreshold = 100 ether;

    uint public constant expertTokensPerEth = 4000;
    ...
    function BTFpreSale(uint _startAtBlock, uint _endAtBlock, uint softCapInEther){
